...
func TestHTTPClientTruncation(t *testing.T) {
	var (
		rotatorSettings = NewRotatorSettings()
		err             error
	)
	...
	rotatorSettings.Prefix = "TRUNCATION"

	// init the HTTP client responsible for recording HTTP(s) requests / responses
	httpClient, errChan, err := NewWARCWritingHTTPClient(HTTPClientSettings{RotatorSettings: rotatorSettings, FullOnDisk: true, MaxReadBeforeTruncate: 1000})
	if err != nil {
		t.Fatalf("Unable to init WARC writing HTTP client: %s", err)
	}
